...
Re-running `pgbouncer:connect` is safe and is the supported way to apply changed credentials (e.g. after rotating the database password): the pgbouncer app is restarted with the current values parsed from `DATABASE_URL`.
...
- Database passwords containing quotes, whitespace, `=` or `\` are rejected, because the pgbouncer image writes the password unquoted into its generated `pgbouncer.ini`. Passwords generated by dokku-postgres are plain hex and always safe.
