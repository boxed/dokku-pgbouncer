...
The teardown always runs to completion. If your app cannot be restarted — scaled to zero, never deployed, no longer building — the command warns and carries on rather than aborting, because the remaining steps are the only thing that releases the postgres service's `post-start-network`. Detaching the shared network and removing it are treated the same way: a failure there is reported and left for a re-run instead of stopping the release, and the closing message then names what is still outstanding rather than claiming a clean finish. The one step that does abort is removing the `PGBOUNCER_*` variables, because nothing has been torn down at that point and continuing would destroy the pooler your app is still pointed at. It also works when the app itself is already gone, and it follows `PGBOUNCER_HOST` rather than the `<app>-pgbouncer` naming convention, so it still finds the right pgbouncer app after an `apps:rename`.
